    ...
    public function build(): \ArielMejiaDev\LarapexCharts\LineChart
    {
        return $this->chart->lineChart()
            ->addData('Total Sales', [
                ...
                round(Purchase::whereBetween('created_at', ['2023-07-01', '2023-07-31'])->sum('totalAmount')),
                round(Purchase::whereBetween('created_at', ['2023-08-01', '2023-08-31'])->sum('totalAmount'))
            ])
            ->addData('Cooked Food Total Sales', [
                ...
                Order::whereHas('purchase', function ($query) {
                    $query->whereBetween('created_at', ['2023-07-01', '2023-07-31']);
                })->whereHas('food', function ($query) {
                    $query->where('type', 3);
                })->sum('amount'),
                Order::whereHas('purchase', function ($query) {
                    $query->whereBetween('created_at', ['2023-08-01', '2023-08-31']);
                })->whereHas('food', function ($query) {
                    $query->where('type', 3);
                })->sum('amount'),
            ])
