...
class FoodController extends Controller
{
        ...
        if ($request->ajax()) {
            return DataTables::of($foods)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="javascript:void(0)" data-toggle="tooltip" title="Food Image" data-id="' . $row->id . '" data-original-title="Image" class="imgBtn btn btn-primary btn-sm viewFoodImg"><i class="bi bi-card-image"></i></a>';

                    $btn = $btn . ' <a href="javascript:void(0)" data-toggle="tooltip" title="Food Nutritional Info" data-id="' . $row->id . '" data-original-title="Data" class="data btn btn-info btn-sm viewFood"><i class="bi bi-info-lg"></i></a>';
                    ...
                    $btn = $btn . ' <a href="/admin/foods/' . $row->id . '/edit" data-toggle="tooltip" title="Edit Food Nutritional Info" data-id="' . $row->id . '" data-original-title="Edit" class="edit btn btn-success btn-sm"><i class="bi bi-pencil-square"></i></a>';

                    $btn = $btn . ' <a href="/admin/foods/' . $row->id . '/delete" data-toggle="tooltip" title="Archive Food Item" data-id="' . $row->id . '" data-original-title="Delete" class="archiveBtn btn btn-warning btn-sm"><i class="bi bi-archive"></i></a>';
                    return $btn;
                })
                // ->addColumn('checkbox',
    ...
    {
        $food = Food::where('id', $id)->first();
        Alert::success('Success', $food->name . ' ' . ' Archived');
        $food->delete();
        ...
        if ($request->ajax()) {
            return DataTables::of($foods)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = ' <a href="/admin/foods/' . $row->id . '/restore" data-toggle="tooltip" title="Restore Food Item" data-id="' . $row->id . '" data-original-title="Restore" class="restoreFood btn btn-success btn-sm"><i class="bi bi-arrow-clockwise"></i></a>';
                    // $btn = $btn . ' <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal"><i class="bi bi-info-lg"></i></button>';
